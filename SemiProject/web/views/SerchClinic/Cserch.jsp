<%@page import="com.kh.dental.searchclinic.model.vo.PageInfo"%>
<%@page import="com.kh.dental.searchclinic.model.vo.SearchClinic"%>
...
<script type="text/javascript">
	function sojaeji(sido, gugun, dong) {
		...
		function makeOption(idx, obj) {
			var html = '<option value="">' + obj.options[0].text;
			+'</option>';
...
<!-- 지도 API 사용 -->
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=f2b66fcd33b13a9b8f7228720a419582&libraries=services"></script>
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=f2b66fcd33b13a9b8f7228720a419582"></script>
...
<body>
	...
	<div>
		<div class="first-div">
			<span class="f17 span-ji">지역</span>&nbsp;&nbsp; <select
				class="select-tag f17" name="sido" id="sido">
				<option class="f17" value="">-시도-</option>
			</select> <select class="select-tag f17" name="gugun" id="gugun">
				<option class="f17" value="">-구군-</option>

			</select> <select class="select-tag f17" name="dong" id="dong">
				<option class="f17" value="">-동-</option>
			</select>
			<button type="button" id="selectsearch">검색하기</button>
	...
	<div>
		<div class="first-div map-tag f20" id="map"></div>
	</div>
	<span id="centerAddr" name="centerAddr1"></span>
	...
	<script type="text/javascript">
					...
					if (result[i].region_type === 'H') {
						...
						$.ajax({
							...
							success : function(data) {
								listSearchClinic = data;
								listCount = data[0].listCount;
								currentPage = data[0].currentPage;
								maxPage = data[0].maxPage;
								startPage = data[0].startPage;
								endPage = data[0].endPage;
								limit = data[0].limit;
								...
								for (var k = 1; k <= limit; k++) {
									positions.push({
										content : '<div>' + data[k].yadm_nm + '</div>',
										latlng : new daum.maps.LatLng(data[k].y_pos, data[k].x_pos)
									});
								...
								$('#div-tag-add-a-tag').append("<div class='w3-bar'>");
								if (currentPage <= 1) {
									$('#div-tag-add-a-tag').append("<a onclick='ataglist1();' id='starttag' class='w3-button' value='<' disabled>&laquo;</a>");
								} else {
									$('#div-tag-add-a-tag').append("<a onclick='ataglist1();' id='starttag' class='w3-button' value='<'>&laquo;</a>");
								}
								for (var p = startPage; p <= endPage; p++) {
									if (p == currentPage) {
										$('#div-tag-add-a-tag').append("<a onclick='ataglist2(" + p + ");' id='numtag' class='w3-button' value='" + p + "'>" + p + "</a>");
									} else {
										$('#div-tag-add-a-tag').append("<a onclick='ataglist2(" + p + ");' id='numtag' class='w3-button' value='" + p + "'>" + p + "</a>");
									}
								...
								if (currentPage >= maxPage) {
									$('#div-tag-add-a-tag').append("<a onclick='ataglist3();' id='endtag' class='w3-button' value='>' disabled>&raquo;</a>");
								} else {
									$('#div-tag-add-a-tag').append("<a onclick='ataglist3();' id='endtag' class='w3-button' value='>' >&raquo;</a>");
								}
								$('#div-tag-add-a-tag').append("</div>");
								$('#div-tag-add-a-tag').append("</div>");
		...
		function ataglist2(p) {
			console.log("클릭이벤트는 가능??");
			console.log("p값은 들어오니???"+p);
			$('#div-tag-table-add').empty();
			if (p > 1) {
				hideMarkers();
				var start = limit * p + 2;
				var end = start + 10;
				console.log("listSearchClinic값은 .잘 있니?????"+listSearchClinic);
	...
	<div class="">
		<div class="w1200 middle" id="div-tag-table-add"></div>
		<div id="div-tag-add-a-tag" align="center"></div>
	</div>
