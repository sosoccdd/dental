...
<!-- 지도 API 사용 -->
<script type="text/javascript"
	src="//dapi.kakao.com/v2/maps/sdk.js?appkey=f2b66fcd33b13a9b8f7228720a419582&libraries=services"></script>
<script type="text/javascript"
	src="//dapi.kakao.com/v2/maps/sdk.js?appkey=f2b66fcd33b13a9b8f7228720a419582"></script>
...
<body>
	                ...
	                $.ajax({
	    				...
	    				success:function(data){
							...
							for (var k = 0; k < positions.length; k++) {
							    ...
							    marker =new daum.maps.Marker({ position : positions[k].latlng });
							    marker.setMap(map);
							    markers.push(marker);
	    					...
	    					for(var k=0;k<data.length;k++){
	    						...
	    						$('#div-tag-table-add').append("<table class='table-box bluck-in' id='table-tag-id'>"
	    							...
	    							+data[k].telno
	    							+"</td></tr><tr><td></td><td>진료시간 &nbsp;:&nbsp;</td>"
	    							+"<td>월-금 9:00 - 21:00 토요일 9:00-14:00 일요일 휴무</td></tr>"
	    							+"<tr><td></td><td>전문분야 &nbsp;:&nbsp;</td><td>치아보철과,치주과,구강내과,치아교정과</td></tr></table>");
	    					
	    					
	    					}
	    					
	    					//$('#div-tag-table-add').html(table);
							...
							geocoder.addressSearch(address, function(result, status) {
								 ...
								 if (status === daum.maps.services.Status.OK) {
									 ...
									 coords = new daum.maps.LatLng(result[0].y, result[0].x);
								        ...
								        // 인포윈도우로 장소에 대한 설명을 표시합니다
								        var infowindow = new daum.maps.InfoWindow({
								            content: '<div style="width:150px;text-align:center;padding:6px 0;">아오</div>'
								        });
								        infowindow.open(map, marker);
	...
	<div class="">
		<div class="w1200 middle" id="div-tag-table-add">
	...
	<% 
    ArrayList<SearchClinic> list = (ArrayList<SearchClinic>)request.getAttribute("list"); 
   ...
   if(currentPage <= 1){ %>
         ...
         <% }else{ %>
         <button onclick="location.href='<%= request.getContextPath()%>/selectsearch.sc?currentPage=<%= currentPage - 1%>'"> < </button>
         ...
         <% for(int p = startPage; p<= endPage; p++){ 
               ...
               <%}else{ %>
                  <button onclick="location.href='<%= request.getContextPath()%>/selectsearch.sc?currentPage=<%= p%>'"><%=p %></button>         
              <%} %>      
         <%} %>
         ...
         <%}else{ %>
            <button onclick="location.href='<%= request.getContextPath()%>/selectsearch.sc?currentPage=<%= currentPage + 1%>'"> > </button>
         <%} %>
         <button onclick="location.href='<%=request.getContextPath() %>/selectsearch.sc?currentPage=<%= maxPage %>'"> >> </button>
